<template>
    <div class="flex items-center px-5 py-3 bg-[#F5F8FC] text-[#1B3764]">
        <div class="flex items-center gap-5 w-[35%]">
            <input type="checkbox" :checked="book.isChosen" @click="handleCheckboxChange">
            <img class="w-[100px]" :src="book.image">
            <div class="text-[16px] font-semibold">{{book.title}}</div>
        </div>
        <div class="w-[20%] text-[14px] font-semibold">{{book.price}}</div>
        ...
        <div class="w-[20%] text-[14px] font-semibold">{{book.total_price}}</div>
        <button class="w-[5%]">
